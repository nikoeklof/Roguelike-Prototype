...
func on_cast_apply(context: CombatContext, item_instance: ItemInstance) -> void:
	...
	var origin := (context.owner as Node2D).global_position
	var space := context.owner.get_world_2d().direct_space_state
	if space == null:
		return
	...
	for h in hits:
		...
		_start_clear_timer(n, duration_sec, func():
