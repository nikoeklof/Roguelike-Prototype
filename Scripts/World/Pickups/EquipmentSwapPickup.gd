...
@export_range(8, 64, 1) var tooltip_title_font_size: int = 16
...
func _play_throw_from_to(interactor: Node, target_origin: Vector2) -> void:
	...
	var start_pos: Vector2 = target_origin
	...
	var end_pos: Vector2 = target_origin + offset
...
func _find_equipment_recursive(root: Node) -> Equipment:
	...
	var direct: Node = root.get_node_or_null("Equipment")
	...
	for child in children:
		...
		if c == null:
			continue
		var found: Equipment = _find_equipment_recursive(c)
		if found != null:
			return found
	return null
...
func _find_item_child() -> Node:
	...
	for child2 in children:
		...
		if n2 == null:
			continue
		if n2 == _tooltip_panel:
			continue
		if n2 is CollisionShape2D:
			continue
		if n2 is Sprite2D:
			continue
		if n2 is Control:
